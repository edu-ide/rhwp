...
test('뮤테이션 표면 원장: 신규·증가 사이트는 baseline 갱신을 강제한다', () => {
  ...
  for (const rel of scanFiles()) {
    let body = source(rel);
    if (rel === 'src/engine/input-handler.ts') {
      // Office 협업 재생은 상대방 연산이므로 로컬 undo stack 에 넣지 않는다.
      // 로컬 33곳의 기준은 유지하고, 아래 별도 가드로 기존 원격 24곳을 동결한다.
      const start = body.indexOf('  applyRealtimeOperation(');
      const end = body.indexOf('  async executeDocumentAgentOperation(', start);
      assert.ok(start >= 0 && end > start, '협업 재생 경계가 있어야 한다');
      assert.equal(mutatorCallCount(body.slice(start, end)), 24,
        '원격 재생의 문서 변경 표면 증가는 별도 검토가 필요하다');
      body = body.slice(0, start) + body.slice(end);
    }
    const n = mutatorCallCount(body);

...
test('embed runtime은 parent의 exact origin에서 v1 port session을 설치한다', async () => {
  ...
  const hostWindow = {
    addEventListener(_type: string, listener: (event: MessageEvent) => void) { if (_type === 'message') messageListener = listener; },
    removeEventListener() {},
  };
...
test('embed runtime은 custom scheme 최상위 same-window legacy 요청을 처리한다', async () => {
  ...
  const hostWindow = {
    addEventListener(_type: string, listener: (event: MessageEvent) => void) {
      if (_type === 'message') messageListener = listener;
    },
    removeEventListener() {},
...
test('Office opaque embedding requires the parent handshake, gates RPC capability, and disposes event subscriptions', async () => {
  ...
  const host = {
    addEventListener(type: string, fn: (event: Event) => void) {
      ...
      else localListeners.set(type, fn);
    },
    removeEventListener(type: string) { localListeners.delete(type); },
  };
  const hostCommand = (detail: unknown = { command: 'save' }) => {
    localListeners.get('rhwp-host-command')?.(new CustomEvent('rhwp-host-command', { detail }));
  };
  const parent = { postMessage() { throw new Error('legacy window response is forbidden'); } };
  const cleanup = installEmbedRuntime({
  ...
  function port() {
    return { onmessage: null as any, closed: false, messages: [] as any[], start() {},
      postMessage(message: unknown) { this.messages.push(message); }, close() { this.closed = true; } };
  }
  function connect(target: ReturnType<typeof port>, source: unknown, capabilities: string[], sessionId = 'office', ports = [target]) {
    listener({ data: { type: 'rhwp-connect', version: 1, sessionId, capabilities },
      source, origin: 'null', ports } as unknown as MessageEvent);
  }
  hostCommand(); // No bound parent: never send a window fallback.
  const invalid = port(); connect(invalid, {}, ['transferable-array-buffer', 'office-realtime-v1']);
  assert.equal(invalid.closed, true);
  const surplus = port(); const extra = port(); connect(surplus, parent, ['transferable-array-buffer'], 'office', [surplus, extra]);
  assert.equal(surplus.closed, true); assert.equal(extra.closed, true);
  listener({ data: { type: 'rhwp-request', id: 1, method: 'dispatchCommand', params: {} }, source: parent, origin: 'null', ports: [] } as unknown as MessageEvent);
  assert.equal(calls, 0);
  const first = port(); connect(first, parent, ['transferable-array-buffer']);
  hostCommand();
  assert.equal(first.messages.length, 1, 'host commands require the Office capability');
  await first.onmessage({ data: { type: 'rhwp-request', version: 1, sessionId: 'office', id: 2,
  ...
  hostCommand();
  emitOperation({ kind: 'insertText', text: '!' });
  assert.deepEqual(replacement.messages.slice(-3).map(({ event, payload }) => ({ event, payload })), [
    { event: 'operation', payload: { kind: 'insertText', text: '안녕' } },
    { event: 'host-command', payload: { command: 'save' } },
    { event: 'operation', payload: { kind: 'insertText', text: '!' } },
  ], 'save must follow earlier edits on the same port, while later edits remain distinguishable');
  const beforeInvalid = replacement.messages.length;
  for (const detail of [null, 'save', {}, { command: 'delete' }]) hostCommand(detail);
  assert.equal(replacement.messages.length, beforeInvalid);
  assert.equal(first.messages.some(message => message.event === 'host-command'), false);
  const wrongSession = port(); connect(wrongSession, parent, ['transferable-array-buffer', 'office-realtime-v1'], 'other');
...
test('Office RPC rejects legacy transport even for a valid HTTP parent', async () => {
  let calls = 0;

...
import { InsertTextCommand, DeleteTextCommand, SplitParagraphCommand, MergeParagraphCommand, MergeNextParagraphCommand, SplitParagraphInCellCommand, MergeParagraphInCellCommand, MergeNextParagraphInCellCommand, MoveTableCommand, MovePictureCommand, MoveShapeCommand, ResizeObjectCommand, DeleteSelectionCommand, ApplyCharFormatCommand, ApplyParaFormatCommand, SnapshotCommand, SubmodeSnapshotCommand, SubmodeSelectionSnapshotCommand, SetFormValueCommand, TextMutationEffectAccumulator, IMMEDIATE_TEXT_MUTATION_EFFECTS, applyCharShapeModsToRange, cellAxisPath, cellParaIndexOf } from './command';
...
export class InputHandler {
  ...
  executeOperation(desc: OperationDescriptor): void {
    ...
    switch (desc.kind) {
      case 'command': {
        ...
        if (!desc.command.isNoOp?.() && desc.meta?.realtimeOperation) {
  ...
  private buildDeleteObjectRealtimeOperation(ref: Omit<RealtimeObjectRef, 'type'> & { type: RealtimeObjectRef['type'] | 'ole' }): RhwpRealtimeOperationDraft | undefined {
    if (ref.headerFooter || ref.type === 'ole') return undefined;
    return {
      kind: 'deleteObject',

...
export async function routeEmbedRequest(
  ...
  switch (method) {
    ...
    default:
      if (OFFICE_EMBED_METHODS.has(method) && handlers.officeRequest) {
        if (allowLegacyArray) throw new Error('Office methods require authenticated MessagePort transport.');
        return handlers.officeRequest(method, params);
      }

    ...
    try {
      ...
      if (ih) {
        ih.executeOperation({
          ...
          operation: () => { commit(); committed = true; return ih.getCursorPosition(); },
        });
      } else {
        commit();
        committed = true;
      ...
      if (committed) {
        const length = this.wasm.getCellParagraphLength(this.ctx.sec, this.ctx.ppi, this.ctx.ci, this.ctx.cellIndex, 0);
        const actual = this.wasm.getTextInCell(this.ctx.sec, this.ctx.ppi, this.ctx.ci, this.ctx.cellIndex, 0, 0, length);
        this.emitCellTextReplacementRealtimeOperations(oldText, actual);
      }
      return committed; // 성공 → 대화상자 닫기
    } catch (e: any) {

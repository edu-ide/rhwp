...
test('opaque srcdoc handshake retries delayed Studio startup and keeps operation events on the port', async () => {
  let attempts = 0;
  let server;
  let session;
  let legacyListeners = 0;
  const transport = new EditorTransport({
    hasAttribute: name => name === 'srcdoc',
    contentWindow: { postMessage(message, origin, ports) {
      attempts += 1;
      assert.equal(origin, '*');
      assert.equal(message.type, 'rhwp-connect');
      if (attempts < 3) { ports[0].close(); return; }
      server = ports[0];
      session = message.sessionId;
      server.start();
      server.postMessage({ type: 'rhwp-connected', version: 1, sessionId: session,
        capabilities: ['transferable-array-buffer', 'office-realtime-v1'] });
    } },
  }, 'https://studio.example/', {
    opaqueOrigin: true, handshakeRetryMs: 5, handshakeTimeoutMs: 200,
    window: { addEventListener() { legacyListeners++; }, removeEventListener() {} },
  });
  try {
    const connecting = transport.connect();
    assert.equal(transport.connect(), connecting);
    await connecting;
    assert.equal(attempts, 3);
    const operation = new Promise(resolve => transport.on('operation', resolve));
    server.postMessage({ type: 'rhwp-event', version: 1, sessionId: session,
      event: 'operation', payload: { kind: 'insertText', text: '안녕' } });
    assert.deepEqual(await operation, { kind: 'insertText', text: '안녕' });
    assert.equal(legacyListeners, 0);
    await transport.connect();
    assert.equal(attempts, 3);
  } finally { transport.destroy(); server?.close(); }
});

test('opaque srcdoc handshake timeout never sends legacy window requests', async () => {
  let attempts = 0;
  let legacyListeners = 0;
  const transport = new EditorTransport({
    hasAttribute: name => name === 'srcdoc',
    contentWindow: { postMessage(message, _origin, ports) {
      assert.equal(message.type, 'rhwp-connect'); attempts++; ports[0].close();
    } },
  }, 'https://studio.example/', {
    opaqueOrigin: true, handshakeRetryMs: 5, handshakeTimeoutMs: 25,
    window: { addEventListener() { legacyListeners++; }, removeEventListener() {} },
  });
  try {
    await assert.rejects(transport.connect(), /requires MessagePort/);
...
test('Office save and operation events share FIFO delivery and require the Office capability', { timeout: 2_000 }, async () => {
  for (const office of [true, false]) {
    let server;
    let sessionId;
    const transport = new EditorTransport({ contentWindow: {
      postMessage(message, _origin, ports) {
        server = ports[0]; sessionId = message.sessionId;
        server.start();
        server.postMessage({ type: 'rhwp-connected', version: 1, sessionId,
          capabilities: ['transferable-array-buffer', 'document-change-events-v1', ...(office ? ['office-realtime-v1'] : [])] });
      },
    } }, 'https://studio.example/', { window: { addEventListener() {}, removeEventListener() {} } });
    try {
      await transport.connect();
      const received = [];
      transport.on('operation', payload => received.push(['operation', payload]));
      transport.on('host-command', payload => received.push(['host-command', payload]));
      const delivered = new Promise(resolve => transport.on('documentChanged', resolve));
      const send = (event, payload) => server.postMessage({ type: 'rhwp-event', version: 1, sessionId, event, payload });
      send('operation', { sequence: 1 });
      send('host-command', { command: 'save' });
      send('operation', { sequence: 2 });
      send('documentChanged', {}); // FIFO marker: all preceding port events have been processed.
      await delivered;
      assert.deepEqual(received, office ? [
        ['operation', { sequence: 1 }], ['host-command', { command: 'save' }], ['operation', { sequence: 2 }],
      ] : []);
    } finally { transport.destroy(); server?.close(); }
  }
});

test('explicitly sandboxed HTTP Studio uses only the opaque MessagePort handshake', async () => {
  let server;
  let legacyListeners = 0;
  const transport = new EditorTransport({
    hasAttribute: name => name === 'sandbox',
    getAttribute: name => name === 'sandbox' ? 'allow-scripts allow-forms allow-modals allow-downloads' : null,
    contentWindow: { postMessage(message, origin, ports) {
      assert.equal(origin, '*');
      assert.equal(message.type, 'rhwp-connect');
      server = ports[0];
      ...
      server.start();
      server.postMessage({ type: 'rhwp-connected', version: 1, sessionId: message.sessionId,
        capabilities: ['transferable-array-buffer'] });
    } },
  }, 'https://studio.example/', {
    opaqueOrigin: true, handshakeTimeoutMs: 100,
    window: { addEventListener() { legacyListeners++; }, removeEventListener() {} },
  });
  try {
    await transport.connect();
    assert.equal(await transport.request('ready'), true);
    assert.equal(legacyListeners, 0);
  } finally { transport.destroy(); server?.close(); }
});

test('opaque HTTP Studio requires an explicit sandbox without allow-same-origin', () => {
  for (const sandbox of [null, 'allow-scripts allow-same-origin', 'ALLOW-SAME-ORIGIN allow-scripts']) {
    assert.throws(() => new EditorTransport({
      hasAttribute: name => name === 'sandbox' && sandbox !== null,
      getAttribute: name => name === 'sandbox' ? sandbox : null,
    }, 'https://studio.example/', { opaqueOrigin: true }), /srcdoc|sandbox/);
  }
});

test('local scheme requires explicit sameOrigin and matching parent protocol and host', () => {
  const previous = globalThis.location;
  globalThis.location = new URL('tauri://localhost/app');
  const options = { window: { removeEventListener() {} } };
  try {
    assert.throws(() => new EditorTransport({}, 'tauri://localhost/studio', options), /HTTP\(S\)/);
    assert.throws(() => new EditorTransport({}, 'tauri://other/studio', { ...options, sameOrigin: true }), /HTTP\(S\)/);
    const transport = new EditorTransport({}, 'tauri://localhost/studio', { ...options, sameOrigin: true });
    assert.equal(transport._targetOrigin, 'tauri://localhost');
    transport.destroy();
    assert.throws(() => new EditorTransport({ hasAttribute: () => false }, 'https://studio.example', {
      ...options, opaqueOrigin: true,
    }), /srcdoc/);
  } finally {

...
export class EditorTransport {
  constructor(iframe, studioUrl, options = {}) {
    ...
    const parentUrl = globalThis.location?.href ? new URL(globalThis.location.href) : null;
    const localSameOrigin = options.sameOrigin === true && parentUrl
      && ['tauri:', 'asset:', 'app:', 'alhangeul-studio:'].includes(targetUrl.protocol)
      && targetUrl.protocol === parentUrl.protocol && targetUrl.host === parentUrl.host;
    if (targetUrl.protocol !== 'http:' && targetUrl.protocol !== 'https:' && !localSameOrigin) {
      throw new Error('studioUrl must use HTTP(S) or an explicitly selected same-origin local scheme');
    }
    this._opaqueOrigin = options.opaqueOrigin === true;
    if (this._opaqueOrigin && !iframe.hasAttribute('srcdoc')) {
      // An HTTP frame can inherit the host's opaque sandbox. Require its own
      // explicit sandbox rather than inferring that guarantee from the parent.
      const sandbox = iframe.hasAttribute('sandbox') ? iframe.getAttribute('sandbox') : null;
      if (sandbox === null || sandbox.toLowerCase().split(/\s+/).includes('allow-same-origin')) {
        throw new Error('opaqueOrigin requires a srcdoc iframe or a sandbox without allow-same-origin');
      }
    }
    this._targetOrigin = this._opaqueOrigin ? '*' : localSameOrigin
      ? `${targetUrl.protocol}//${targetUrl.host}` : targetUrl.origin;
    this._window = options.window || window;
    this._requestTimeoutMs = options.requestTimeoutMs;
    this._handshakeTimeoutMs = options.handshakeTimeoutMs ?? (this._opaqueOrigin ? 30000 : 1000);
    this._handshakeRetryMs = options.handshakeRetryMs ?? 250;
  ...
  _attemptConnect() {
    if (!this._connectResolve || this._destroyed) return;
    ...
    try {
      ...
      if (this._connectResolve) {
        this._retryTimer = setTimeout(() => this._attemptConnect(), this._handshakeRetryMs);
      }
    } catch (error) {
      channel.port2.close();
  ...
  request(method, params = {}, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      }, requestTimeoutFor(method, options.timeoutMs ?? this._requestTimeoutMs));
  ...
  _handlePortMessage(message) {
    ...
    if (message?.type === 'rhwp-event') {
      ...
      if (message.version !== PROTOCOL_VERSION
          || message.sessionId !== this._sessionId
          || !this._peerCapabilities.has(message.event === 'operation' || message.event === 'host-command'
            ? 'office-realtime-v1' : 'document-change-events-v1')
  ...
  _useLegacy() {
    ...
    if (this._opaqueOrigin) { this._rejectConnect(new Error('Opaque Studio requires MessagePort transport')); return; }
    this._port && (this._port.onmessage = null);
    this._port?.close();
    this._port = null;
    clearTimeout(this._retryTimer);

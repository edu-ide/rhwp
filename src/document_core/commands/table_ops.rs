...
impl DocumentCore {
    ...
    pub fn set_cell_properties_native(
        ...
        if needs_reflow {
            // Cell properties explicitly own height and width. Reflow with the table
            // frame plan without the Office paragraph-edit geometry growth hook.
            self.reflow_table_cell_paragraphs(
                section_idx,
                parent_para_idx,
                control_idx,
                &[(cell_idx, reflow_para_count)],
            );
        }
        ...
        for (cell_idx, para_count) in reflow_cells {
            for cell_para_idx in 0..para_count {
                if nested {
                    ...
                    let target = cell_path.last_mut().expect("resolved nonempty table path");
                    target.1 = cell_idx;
                    target.2 = cell_para_idx;

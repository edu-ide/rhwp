...
impl DocumentCore {
    ...
    pub fn apply_para_format_in_cell_native(
        ...
        if mods.spacing_before.is_some() || mods.spacing_after.is_some() {
            self.sync_owner_cell_geometry(sec_idx, parent_para_idx, control_idx, cell_idx);
        }
    ...
    pub fn apply_para_format_in_cell_by_path_native(
        ...
        if para_shape_mods_affect_text_flow(&mods) {
            let inner_cpi = path.last().expect("validated nonempty cell path").2;
            self.reflow_cell_paragraph_by_path(sec_idx, parent_para_idx, path, inner_cpi);
        }

...
impl DocumentCore {
    ...
    fn reflow_cell_paragraph_with_edit(
        ...
        // Incremental text edits preserve stored flow positions for deferred pagination.
        // Explicit paragraph reflow still grows Office form geometry. Table-frame batch
        // reflow bypasses this path so explicit cell dimensions remain authoritative.
        if edit_char_offset.is_none() && cell_idx != 65534 {
            self.sync_owner_cell_geometry(section_idx, parent_para_idx, control_idx, cell_idx);
        }
    }
    ...
    pub(crate) fn sync_owner_cell_geometry(
        ...
        let (table_tac, new_table_height, line_height_extra) = {
            ...
            // A height update must not discard the stored width residual that the
            // upstream paragraph-frame planner assigns to the final column.
    ...
    pub(crate) fn reflow_cell_paragraph_by_path(
        ...
        if let Some(&(outer_ctrl, outer_cell, _)) = path.first() {
